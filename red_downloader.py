...
***
OdyAsh note: Because I want to change the formatting of the downloaded images,
I've taken the file outside of (.env) and placed it here, so you don't have to "pip install RedDownloader"
Full credit of the work here goes to JackhammerYT: https://github.com/JackhammerYT/RedDownloader
***
...
class Download:
        ...
        if quality not in qualityTypes:  # if quality is not in the list
            ...
            if 'v.redd.it' in self.postLink:  # if the post is a video
                ...
                if not self.postLink.endswith('.gif') and not self.postLink.endswith('.GIF') and not self.postLink.endswith('.gifv') and not self.postLink.endswith('.GIFV'):
                    ...
                    file = open(f'{self.output}' + '.jpg', 'wb')
                    file.write(imageData)
                    file.close()
                    #print("Sucessfully Downloaded Image: " + f"{self.output}")
                    if self.destination is not None:
                        shutil.move(f"{self.output}.jpg", self.destination)
                else:
    ...
    def GetGallery(self, posts):  # Function to download and merge all images in a directory
        ...
        for i in range(TotalPosts):
            ...
            with open(os.path.join(self.directory, self.output + f'{i+1}.jpg'), 'wb') as image:
...
class DownloadImagesBySubreddit:
    ...
    def __init__(
        ...
        if SortBy == "hot" or SortBy == "new" or SortBy == "top":
            ...
            try:
                ...
                self.ProcessedLinks = Links
                self.DownloadLinks(
                    self.ProcessedLinks, quality, output, destination, offset=offset)
            except Exception as e:
                print("Unable to fetch posts")
                print(e)
        else:
    ...
    def DownloadLinks(self, links, quality, output, destination, offset=0):
        print("Downloading Posts...")
        # print("Creating ", output, " directory...")
        # path = ''
        # try:
        ...
        #     pass
        length = len(links)
        prevFiles = os.listdir(destination)
        if len(prevFiles) > 0:
        ...
        try:
            for link in links:
                print(f"Downloading Post {i-offset+1} of {length}")
                if destination is not None:
                    Download(
                        link,
                        quality,
